...
{
    LogPrint("masternode", "AddedMasternodePotentialList, %d entries\n", seenNodes.size());
...
void CMasternodeSync::GetNextAsset()
{
    ...
    case (MASTERNODE_SYNC_LIST):
        RequestedMasternodeAssets = MASTERNODE_SYNC_POTENTIAL;
        break;
...
std::string CMasternodeSync::GetSyncStatus()
{
    ...
    case MASTERNODE_SYNC_POTENTIAL:
        return _("Synchronizing potential masternodes...");
    case MASTERNODE_SYNC_MNW:
...
void CMasternodeSync::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (strCommand == "ssc") { //Sync status count
        ...
        case (MASTERNODE_SYNC_POTENTIAL):
            if (nItemID != RequestedMasternodeAssets) return;
	    LogPrint("masternode", "ProcessMessage - Potential\n");
...
void CMasternodeSync::ClearFulfilledRequest()
{
    ...
    for (auto pnode : vNodes) {
        ...
        pnode->ClearFulfilledRequest("mnpsync");
...
void CMasternodeSync::Process()
{
    ...
    if (RequestedMasternodeAssets == MASTERNODE_SYNC_FAILED) {
        if (lastFailure + MASTERNODE_RESET_TIMEOUT < GetTime()) {
            ...
            LogPrint("masternode", "Failed mnsync\n");
            return;
	}
    }
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST &&
        !IsBlockchainSynced() &&
       	RequestedMasternodeAssets > MASTERNODE_SYNC_SPORKS) {
	LogPrint("masternode", "mnsync delayed: Sporks synced, blockchain not\n");
	GetNextAsset();
	return;
    }	
    ...
    TRY_LOCK(cs_vNodes, lockRecv);
    if (!lockRecv) {
	LogPrint("masternode", "mnsync can't lock vNode\n");
	return;
    }
    ...
    for (auto pnode : vNodes) {
        if (Params().NetworkID() == CBaseChainParams::REGTEST) {
            if (RequestedMasternodeAttempt <= 2) {
                ...
                int nMnpCount = mnodeman.PotentialMasternodeCount();
                pnode->PushMessage("mnpget", nMnpCount); //sync payees
                uint256 n = 0;
        ...
        if (pnode->nVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
		    ...
		    LogPrint("masternode", "mnsync timeout\n");
                    GetNextAsset();
		    return;
	        }
            ...
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_POTENTIAL) {
                ...
                if (lastMasternodePotentialList > 0 &&
                    lastMasternodePotentialList < GetTime() - MASTERNODE_SYNC_TIMEOUT * 2 &&
		    RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD) {
		    // hasn't received a new item in the last five seconds, so we'll move to the
                    GetNextAsset();
                    return;
		    ...
		    // continue;
		     ...
		     GetTime() - nAssetSyncStarted > MASTERNODE_SYNC_TIMEOUT * 5)) {
                ...
                if (RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD * 3)
		{
		    LogPrint("masternode", "mnpsync timeout\n");
                    GetNextAsset();
	       	    return;
		}
        ...
        if (pnode->nVersion >= ActiveProtocol()) {
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_BUDGET) {
                ...
                if (lastBudgetItem > 0 &&
                    lastBudgetItem < GetTime() - MASTERNODE_SYNC_TIMEOUT * 2 &&
		    RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD) { 
		    ...
		    continue;
                ...
                if (RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD * 3) {
		    LogPrint("masternode", "busync timeout\n");
                    GetNextAsset();
		    return;
		}
