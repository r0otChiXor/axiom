    ...
    if (strCommand == "mnget") { //Masternode Payments Request Sync
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (pfrom->HasFulfilledRequest("mnget")) {
                LogPrint("masternode", "CMasternodePayments::ProcessMessageMasternodePayments() : mnget - peer already asked me for the list\n");
                //Misbehaving(pfrom->GetId(), 20, "mnget already asked");
                return;
            }
        }
...
bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (n > MNPAYMENTS_SIGNATURES_TOTAL) {
        ...
        if (n > MNPAYMENTS_SIGNATURES_TOTAL * 2) {
            ...
            LogPrint("masternode","CMasternodePaymentWinner::IsValid - %s\n", strError);
            //if (masternodeSync.IsSynced()) Misbehaving(pnode->GetId(), 20, "masternode not in top");
        }
        return false;
    }
